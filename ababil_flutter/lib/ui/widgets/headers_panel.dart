...
class HeadersPanel extends StatefulWidget {
  ...
  final Set<String> disabledHeaders;
  final VoidCallback onAddHeader;
  final ValueChanged<String> onRemoveHeader;
  final ValueChanged<MapEntry<String, String>> onHeaderChanged;
  final ValueChanged<String> onToggleHeaderEnabled;

  const HeadersPanel({
    super.key,
    required this.headers,
    required this.disabledHeaders,
    required this.onAddHeader,
    required this.onRemoveHeader,
    required this.onHeaderChanged,
    required this.onToggleHeaderEnabled,
  });
...
class _HeadersPanelState extends State<HeadersPanel> {
  ...
  Widget build(BuildContext context) {
    ...
    for (var entry in widget.headers.entries) {
            ...
            child: Table(
              ...
              children: [
                ...
                ...widget.headers.entries.map((entry) {
                  final isEnabled = !widget.disabledHeaders.contains(entry.key);
                  return _buildTableRow(
                    ...
                    onChanged: (newEntry) {
                      ...
                      if (key.isEmpty && value.isEmpty) {
                        widget.onRemoveHeader(entry.key);
                      } else if (key != entry.key || value != entry.value) {
                        widget.onRemoveHeader(entry.key);
                        if (key.isNotEmpty && value.isNotEmpty) {
                          widget.onHeaderChanged(MapEntry(key, value));
                        }
                      }
                    },
                    onRemove: () => widget.onRemoveHeader(entry.key),
                    onToggleEnabled: () =>
                        widget.onToggleHeaderEnabled(entry.key),
                  );
                }),
                ...
                _buildTableRow(
                  ...
                  onChanged: (entry) {
                    if (entry.$1.isNotEmpty && entry.$2.isNotEmpty) {
                      widget.onHeaderChanged(MapEntry(entry.$1, entry.$2));
  ...
  TableRow _buildTableRow({
    ...
    if (isNewRow) {
      ...
      if (keyNotifier.value != headerKey) keyNotifier.value = headerKey;
      if (valueNotifier.value != headerValue) valueNotifier.value = headerValue;
    }
...
// Shared state widget for an entire header row
// Uses ValueNotifiers to share state across all cells in the row
class _HeaderRowWidget extends StatefulWidget {
  final String rowId;
  ...
  @override
  State<_HeaderRowWidget> createState() => _HeaderRowWidgetState();
}

class _HeaderRowWidgetState extends State<_HeaderRowWidget> {
  ...
  @override
  void didUpdateWidget(_HeaderRowWidget oldWidget) {
...
class _HeaderKeyCell extends StatefulWidget {
  final String headerKey;
  final String headerValue;
  final bool isNewRow;
  final bool isEnabled;
  ...
  @override
  State<_HeaderKeyCell> createState() => _HeaderKeyCellState();
}

class _HeaderKeyCellState extends State<_HeaderKeyCell> {
  late TextEditingController _controller;

  @override
  void initState() {
    super.initState();
    _controller = TextEditingController(text: widget.headerKey);
  }

  @override
  void didUpdateWidget(_HeaderKeyCell oldWidget) {
    super.didUpdateWidget(oldWidget);
    if (oldWidget.headerKey != widget.headerKey) {
      _controller.text = widget.headerKey;
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: TextField(
        ...
        onChanged: (_) {
          widget.onChanged((_controller.text, widget.headerValue));
        },
      ),
    );
  }
...
class _HeaderValueCell extends StatefulWidget {
  final String headerKey;
  final String headerValue;
  final bool isNewRow;
  final bool isEnabled;
  ...
  @override
  State<_HeaderValueCell> createState() => _HeaderValueCellState();
}

class _HeaderValueCellState extends State<_HeaderValueCell> {
  late TextEditingController _controller;

  @override
  void initState() {
    super.initState();
    _controller = TextEditingController(text: widget.headerValue);
  }

  @override
  void didUpdateWidget(_HeaderValueCell oldWidget) {
    super.didUpdateWidget(oldWidget);
    if (oldWidget.headerValue != widget.headerValue) {
      _controller.text = widget.headerValue;
    }
  }
  ...
  Widget build(BuildContext context) {
    final isDimmed =
        widget.isNewRow && widget.headerKey.isEmpty && _controller.text.isEmpty;
    return Padding(
      padding: const EdgeInsets.symmetric(horizontal: 8),
      child: TextField(
        controller: _controller,
        ...
        onChanged: (_) {
          widget.onChanged((widget.headerKey, _controller.text));
        },
      ),
    );
  }
